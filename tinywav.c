    ...
    case TW_INT16: {
      ...
      switch (tw->chanFmt) {
        ...
        case TW_SPLIT: { // channel buffer is split e.g. [[LLLL],[RRRR]]
          for (int i = 0, pos = 0; i < tw->numChannels; i++) {
            int ofs =i * frames_read;
            for (int j = 0; j < frames_read; j++, ++pos) {
              ((float *) data)[ofs + j] = (float) interleaved_data[j*tw->numChannels + i] / INT16_MAX;
            }
          }
          return frames_read;
        }
        ...
        case TW_SPLIT: { // channel buffer is split e.g. [[LLLL],[RRRR]]
          for (int i = 0, pos = 0; i < tw->numChannels; i++) {
            ...
            for (int j = 0; j < valid_len; j++, ++pos) {
              ((int16_t *) data)[ofs + j] = interleaved_data[j*tw->numChannels + i];
            }
          }
          return valid_len;
        }
    ...
    case TW_FLOAT32: {
      ...
      switch (tw->chanFmt) {
        ...
        case TW_INLINE: { // channel buffer is inlined e.g. [LLLLRRRR]
          for (int i = 0, pos = 0; i < tw->numChannels; i++) {
            for (int j = i; j < valid_len * tw->numChannels; j += tw->numChannels, ++pos) {
              ((int16_t *) data)[pos] = interleaved_data[j] * INT16_MAX;
            }
          }
          return valid_len;
        }
        case TW_SPLIT: { // channel buffer is split e.g. [[LLLL],[RRRR]]
          for (int i = 0, pos = 0; i < tw->numChannels; i++) {
            for (int j = 0; j < valid_len; j++, ++pos) {
              ((int16_t **) data)[i][j] = interleaved_data[j*tw->numChannels + i] * INT16_MAX;
            }
          }
          return valid_len;
        }
